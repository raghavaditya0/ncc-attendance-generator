...
const defaultHods = [
  ...
  { name: "Dr. Seema Singh", department: "1st Year Academics" },
];
...
export default function Home() {
  ...
  const openDirectorySelection = () => {
    const currentIds = directory.filter((cadet) => cadets.some((selected) => selected.name.trim().toLowerCase() === cadet.name.trim().toLowerCase())).map((cadet) => cadet.id);
    setSelectedDirectoryIds(currentIds);
    setDirectorySearch("");
    setShowDirectory(true);
  };
  const selectAllDirectory = () => setSelectedDirectoryIds(directory.map((cadet) => cadet.id));
  const clearDirectorySelection = () => setSelectedDirectoryIds([]);
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <div className="builder-grid no-print"><section className="panel form-panel"><div className="panel-heading"><div><span className="section-kicker">STEP 01 / DETAILS</span><h2>{ui ? "Attendance details" : "Attendance details"}</h2></div><div className="count-pill"><Users size={14} /> {activeCadets.length} present</div></div>
        ...
        <div className="cadet-heading"><div><div className="field-label">{ui ? "Kaun-kaun present hai?" : "Who was present?"}<span className="required">Required</span></div><p>{ui ? "Checkbox se present cadets select karo — unselected cadets absent rahenge." : "Tick the checkbox for each cadet who was present. Unticked cadets stay absent."}</p></div><div className="roll-actions"><button className="text-action" onClick={openDirectorySelection}><Check size={15} /> Select present cadets</button><label className="text-action upload-action"><Download size={15} /> Excel list<input type="file" accept=".xlsx,.xls" onChange={handleExcelUpload} /></label><button className="text-action" onClick={() => setShowImport(true)}><Clipboard size={15} /> Paste names</button></div></div>
    ...
    {showDirectory && <div className="modal-backdrop no-print" onClick={() => setShowDirectory(false)}><div className="modal directory-modal" onClick={(event) => event.stopPropagation()}><div className="modal-header"><div><span className="section-kicker">CADET DIRECTORY</span><h2>Mark today’s attendance</h2></div><button className="icon-button" onClick={() => setShowDirectory(false)}><X size={18} /></button></div><p>{directory.length} cadets available. Tick the checkbox for everyone who came today; leave absent cadets unchecked.</p><input className="text-input directory-search" value={directorySearch} onChange={(event) => setDirectorySearch(event.target.value)} placeholder="Search name, rank, year or regimental no." /><div className="directory-bulk-actions"><button className="text-action" onClick={selectAllDirectory}>Select all</button><button className="text-action" onClick={clearDirectorySelection}>Clear all</button></div><div className="directory-list">{filteredDirectory.map((cadet) => <button key={cadet.id} type="button" aria-pressed={selectedDirectoryIds.includes(cadet.id)} className={`directory-row ${selectedDirectoryIds.includes(cadet.id) ? "selected" : ""}`} onClick={() => toggleDirectoryCadet(cadet.id)}><span className="directory-check" aria-hidden="true">{selectedDirectoryIds.includes(cadet.id) ? <Check size={14} /> : ""}</span><span><strong>{cadet.name}</strong><small>{cadet.rank} · {cadet.year} · {cadet.enrollment}</small></span></button>)}</div><div className="directory-footer"><span>{selectedDirectoryIds.length} present · {Math.max(directory.length - selectedDirectoryIds.length, 0)} absent</span><button className="generate-button" onClick={applyDirectorySelection}>Use selected cadets <ArrowRight size={18} /></button></div></div></div>}
